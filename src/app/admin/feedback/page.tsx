...
export default function AdminFeedbackPage() {
  ...
  useEffect(() => { fetchFeedbacks(); }, [page, typeFilter]);
  ...
  return (
    <div className="min-h-screen py-10">
      ...
      <div className="section-container max-w-5xl">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mb-6">Обратная связь</h1>

        <div className="flex gap-2 mb-6">
          {[{ v: 'all', l: 'Все' }, { v: 'bug', l: 'Ошибки' }, { v: 'feature', l: 'Фичи' }].map(t => (
            <button key={t.v} onClick={() => { setTypeFilter(t.v); setPage(1); }} className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${typeFilter === t.v ? 'bg-brand-500 text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50'}`}>
              {t.l}
            </button>
          ))}
          <span className="ml-auto text-sm text-gray-400 self-center">{total} всего</span>
        </div>
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs text-gray-400">{new Date(f.createdAt).toLocaleString('ru-RU')}</span>
                      {f.user && <span className="text-xs text-gray-500">{f.user.name || f.user.email}</span>}
                    </div>
                    <p className="text-sm text-gray-700 whitespace-pre-wrap">{f.message}</p>
                  </div>
                  <button onClick={() => handleDelete(f.id)} className="p-2 text-gray-400 hover:text-red-500 transition-colors shrink-0">
