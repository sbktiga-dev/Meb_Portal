    ...
    <div className="bg-gray-50 dark:bg-gray-900 min-h-screen py-8">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between mb-6">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Производства ({items.length})</h1>
        ...
        {showForm && (
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-md p-6 mb-6">
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <input type="text" placeholder="Название *" value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} className="px-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:border-amber-500" />
              <input type="text" placeholder="Адрес" value={form.address} onChange={e => setForm(p => ({ ...p, address: e.target.value }))} className="px-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:border-amber-500" />
              ...
              <input type="text" placeholder="Возможности (через запятую)" value={form.capabilities} onChange={e => setForm(p => ({ ...p, capabilities: e.target.value }))} className="px-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:border-amber-500" />
              <input type="text" placeholder="География" value={form.geometry} onChange={e => setForm(p => ({ ...p, geometry: e.target.value }))} className="px-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl focus:outline-none focus:border-amber-500" />
          ...
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-gray-700/50 border-b dark:border-gray-700">
              <tr>
                ...
                <th className="text-left px-6 py-3 text-sm font-medium text-gray-600 dark:text-gray-400">Адрес</th>
                <th className="text-left px-6 py-3 text-sm font-medium text-gray-600 dark:text-gray-400">Возможности</th>
            ...
            <tbody className="divide-y dark:divide-gray-700">
              {items.map(m => (
                <tr key={m.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  <td className="px-6 py-4 text-sm font-medium text-gray-900 dark:text-gray-100">{m.name}</td>
                  <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{m.address || '—'}</td>
                  <td className="px-6 py-4 text-sm text-gray-600 dark:text-gray-400">{m.capabilities.join(', ')}</td>
