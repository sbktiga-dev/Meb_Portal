import { NextRequest, NextResponse } from 'next/server';
export const dynamic = 'force-dynamic';
import { prisma } from '@/lib/prisma';
import { verifyToken } from '@/lib/auth';

export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    const authHeader = req.headers.get('Authorization');
    if (!authHeader?.startsWith('Bearer ')) return NextResponse.json({ error: 'Не авторизован' }, { status: 401 });
    const payload = verifyToken(authHeader.split(' ')[1]);
    if (!payload) return NextResponse.json({ error: 'Невалидный токен' }, { status: 401 });
    const admin = await prisma.user.findUnique({ where: { id: payload.userId }, select: { role: true } });
    if (admin?.role !== 'ADMIN') return NextResponse.json({ error: 'Доступ запрещён' }, { status: 403 });

    const { specialistType } = await req.json();
    const allowedTypes = ['DESIGNER', 'TECHNOLOGIST', 'INSTALLER', 'MANAGER'];
    if (!allowedTypes.includes(specialistType)) {
      return NextResponse.json({ error: 'Невалидная специализация' }, { status: 400 });
    }

    const user = await prisma.user.findUnique({ where: { id: params.id }, select: { specialistId: true } });
    if (!user) return NextResponse.json({ error: 'Пользователь не найден' }, { status: 404 });
    if (!user.specialistId) return NextResponse.json({ error: 'Пользователь не является специалистом' }, { status: 400 });

    await prisma.specialist.update({ where: { id: user.specialistId }, data: { type: specialistType } });
    return NextResponse.json({ specialistType });
  } catch (error) {
    console.error('Admin specialist type error:', error);
    return NextResponse.json({ error: 'Ошибка сервера' }, { status: 500 });
  }
}
