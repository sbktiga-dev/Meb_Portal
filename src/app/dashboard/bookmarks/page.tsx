      ...
      <div className="section-container py-10">
        <div className="flex items-center justify-between mb-8 animate-fade-in">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 mt-1">Коллекции сохранённых материалов</p>
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 animate-fade-in">
            <div className="w-20 h-20 mx-auto mb-6 rounded-2xl bg-gray-100 dark:bg-gray-800 flex items-center justify-center">
              <svg className="w-10 h-10 text-gray-300 dark:text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.5"><path d="M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z"/></svg>
            </div>
            <h3 className="text-xl font-bold text-gray-900 dark:text-gray-100 mb-2">Пока нет закладок</h3>
            <p className="text-gray-500 dark:text-gray-400">Создайте первую коллекцию выше</p>
          </div>
        ) : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {bookmarks.map((bookmark, i) => (
              <div key={bookmark.id} className={`card-base p-5 hover-lift animate-fade-in-up stagger-${Math.min((i % 6) + 1, 6)}`}>
                ...
                <h3 className="font-bold text-gray-900 dark:text-gray-100 text-lg mb-1">{bookmark.name}</h3>
                <p className="text-sm text-gray-400 dark:text-gray-500">{bookmark._count.items} элементов</p>
              </div>
            ))}
          </div>
        )}
