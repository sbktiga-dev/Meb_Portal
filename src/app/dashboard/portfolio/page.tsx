            ...
            {items.map((item, i) => {
              ...
              return (
                <div key={item.id} className={`card-base overflow-hidden animate-fade-in-up stagger-${Math.min(i + 1, 6)}`}>
                  <Link href={`/dashboard/portfolio/${item.id}/edit`} className="block">
                  ...
                  <div className="p-3 sm:p-4">
                    <div className="flex items-start justify-between mb-1.5">
                      ...
                      {item.category && <span className="badge-brand text-[9px] shrink-0 ml-1">{categoryLabels[item.category] || item.category}</span>}
                    </div>
                    {tags.length > 0 && (
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <button onClick={(e) => { e.preventDefault(); handleDelete(item.id); }} disabled={deleting === item.id}
                          className="text-[10px] text-red-400 hover:text-red-600 transition-colors disabled:opacity-40">
