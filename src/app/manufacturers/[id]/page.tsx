...
export default function ManufacturerDetailPage() {
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.ok && data.url) {
        const updateRes = await fetch(`/api/manufacturers/${manufacturer.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
          body: JSON.stringify({ logo: data.url }),
        });
        if (updateRes.ok) {
          setManufacturer(prev => prev ? { ...prev, logo: data.url } : prev);
        }
      }
    } catch {}
  ...
  return (
    <div className="min-h-screen bg-gray-50/50">
      <PageSEO title={manufacturer.name || 'Производство'} description={manufacturer.description?.slice(0, 160) || `Производство на МебПортал: ${manufacturer.name}`} />
      <div className="section-container py-10 max-w-4xl">
        ...
        <div className="card-base overflow-hidden animate-fade-in-up stagger-1">
            ...
            <div className="relative flex items-center gap-4 sm:gap-5">
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mt-2 flex-wrap">
                  {manufacturer.isVerified && <span className="inline-flex items-center gap-1 badge bg-emerald-500/20 text-emerald-300 border border-emerald-400/30 text-xs"><svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2"><polyline points="20 6 9 17 4 12"/></svg>Проверено</span>}
          ...
          <div className="p-4 sm:p-8">
            {manufacturer.description ? <p className="text-gray-600 leading-relaxed mb-6 animate-fade-in-up stagger-2">{manufacturer.description}</p> : <p className="text-gray-400 italic mb-6 animate-fade-in-up stagger-2">Описание не указано</p>}

            {manufacturer.userId && (
              <div className="flex flex-wrap gap-3 mb-6 animate-fade-in-up stagger-3">
                <Link href={`/portfolio/${manufacturer.userId}`} className="btn-primary !px-5 !py-2.5 text-sm">
                ...
                <Link href={`/feed?authorId=${manufacturer.userId}`} className="btn-secondary !px-5 !py-2.5 text-sm">
            ...
            {manufacturer.capabilities.length > 0 && (
              <div className="mb-6 animate-fade-in-up stagger-3">
                <h2 className="font-bold text-gray-900 mb-3">Технические возможности</h2>
                <div className="flex flex-wrap gap-2">
                  {manufacturer.capabilities.map(cap => <span key={cap} className="badge-brand">{cap}</span>)}
                </div>
              </div>
            )}
            ...
            {manufacturer.geometry && (
              <div className="mb-6 animate-fade-in-up stagger-3">
                <h2 className="font-bold text-gray-900 mb-3">География</h2>
                <p className="text-gray-600">{manufacturer.geometry}</p>
              </div>
            )}
            ...
            <div className="grid md:grid-cols-2 gap-3">
              {manufacturer.address && (
                <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl hover:bg-gray-100/70 transition-colors animate-fade-in-up stagger-4">
                  ...
                  <div><p className="text-xs text-gray-400">Адрес</p><p className="font-medium text-gray-900">{manufacturer.address}</p></div>
                </div>
              )}
              {manufacturer.phone && (
                <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl hover:bg-gray-100/70 transition-colors animate-fade-in-up stagger-4">
                  ...
                  <div><p className="text-xs text-gray-400">Телефон</p><p className="font-medium text-gray-900">{manufacturer.phone}</p></div>
                </div>
              )}
              {manufacturer.email && (
                <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl hover:bg-gray-100/70 transition-colors animate-fade-in-up stagger-5">
                  ...
                  <div><p className="text-xs text-gray-400">Email</p><p className="font-medium text-gray-900">{manufacturer.email}</p></div>
                </div>
              )}
              {manufacturer.website && (
                <div className="flex items-center gap-3 p-4 bg-gray-50 rounded-xl hover:bg-gray-100/70 transition-colors animate-fade-in-up stagger-5">
                  ...
                  <div><p className="text-xs text-gray-400">Сайт</p><a href={manufacturer.website} target="_blank" rel="noopener noreferrer" className="font-medium text-brand-600 hover:text-brand-700">{manufacturer.website}</a></div>
                </div>
              )}
            </div>
          </div>
