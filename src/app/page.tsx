...
function AnimatedCounter({ value, suffix = '' }: { value: number; suffix?: string }) {
  ...
  return <div ref={ref} className="text-4xl md:text-5xl font-bold text-brand-500 tabular-nums">{count}{suffix}</div>;
}

export default function HomePage() {
  const [stats, setStats] = useState<StatsData | null>(null);
  const [recentImages, setRecentImages] = useState<ImageData[]>([]);
  const [popularDocs, setPopularDocs] = useState<DocumentData[]>([]);
  const [feedPosts, setFeedPosts] = useState<PostData[]>([]);

  useEffect(() => {
    fetch('/api/stats').then(r => r.json()).then(d => setStats(d.stats)).catch(() => {});
    fetch('/api/images?limit=6&sort=downloads').then(r => r.json()).then(d => setRecentImages(d.images || [])).catch(() => {});
    fetch('/api/documents?limit=5').then(r => r.json()).then(d => setPopularDocs(d.documents || [])).catch(() => {});
    fetch('/api/feed?limit=4&sort=newest').then(r => r.json()).then(d => setFeedPosts(d.posts || [])).catch(() => {});
  }, []);
        ...
        <section className="section-padding section-container">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 md:gap-6">
            {recentImages.map((img, i) => (
              <Link key={img.id} href={`/gallery/${img.id}`} className={`card-base overflow-hidden animate-fade-in-up stagger-${i + 1}`}>
        ...
        <section className="section-padding section-container">
          ...
          <div className="max-w-2xl mx-auto space-y-6">
            {feedPosts.map((post, i) => {
              ...
              return (
                <article key={post.id} className={`card-base overflow-hidden animate-fade-in-up stagger-${i + 1}`}>
                  <div className="flex items-center gap-3 p-4 pb-0">
                    {post.author.avatar ? (
                      <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-white shadow-sm flex-shrink-0">
                        <Image src={post.author.avatar} alt="" width={40} height={40} className="w-full h-full object-cover" unoptimized />
                      </div>
                    ) : (
        ...
        <div className="section-container">
          ...
          <div className="grid md:grid-cols-3 gap-6 md:gap-8">
            {[
              ...
              <Link key={feature.href} href={feature.href} className={`card-base p-8 text-center hover-lift animate-fade-in-up stagger-${i + 1}`}>
          ...
          <div className="space-y-3">
            {popularDocs.map(doc => {
              ...
              const icon = doc.fileType === 'pdf'
                ? <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.5"><path d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
                : doc.fileType === 'xlsx'
                ? <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M9 3v18M3 9h18M3 15h18"/></svg>
              ...
              return (
                <Link key={doc.id} href={`/documents/${doc.id}`} className="card-base flex items-center justify-between p-5 hover-lift">
                  ...
                  <div className="hidden sm:flex items-center gap-2 text-brand-600 text-sm font-semibold opacity-0 group-hover:opacity-100 transition-opacity">
                    Скачать
        ...
        <div className="section-container">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <div key={i} className={`card-base p-7 animate-fade-in-up stagger-${i + 1}`}>
