...
export default function SpecialistsContent({
  ...
  const updateParams = useCallback((type: string, sort: string, searchValue: string) => {
    ...
    router.push(`/specialists${query ? `?${query}` : ''}`);
  }, [router]);
      ...
      <div className="section-container py-10 md:py-14">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-card p-5 md:p-6 mb-8">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input
                ...
                onChange={e => {
                  ...
                  updateParams(selectedType, sortBy, e.target.value);
                }}
            ...
            <select value={sortBy} onChange={e => {
              ...
              updateParams(selectedType, e.target.value, search);
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {types.map(t => (
              <button
                ...
                onClick={() => {
                  setSelectedType(t);
                  updateParams(t, sortBy, search);
                }}
                className={`${selectedType === t ? 'filter-chip-active' : 'filter-chip-inactive'} inline-flex items-center gap-1.5`}
              >
                {t !== 'Все' && typeIcons[t]}
        ...
        {specialists.length === 0 ? (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
            {specialists.map(s => (
              <Link key={s.id} href={`/profile/${s.user?.id || s.id}`} className="card-base p-6 hover-lift group">
