...
export default function SuppliersContent({
  ...
  const updateParams = useCallback((category: string, sort: string, searchValue: string) => {
    ...
    router.push(`/suppliers${query ? `?${query}` : ''}`);
  }, [router]);
  ...
  const handleSortChange = (sort: string) => {
    setSortBy(sort);
    updateParams(selectedCategory, sort, search);
  };

  const handleSearch = (value: string) => {
    setSearch(value);
    updateParams(selectedCategory, sortBy, value);
  };
      ...
      <div className="section-container py-10 md:py-14">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-card p-5 md:p-6 mb-8">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <select value={sortBy} onChange={e => handleSortChange(e.target.value)} className="input-premium !w-auto !py-2.5">
        ...
        {suppliers.length === 0 ? (
          <div className="text-center py-20">
            ...
            <h3 className="text-xl font-semibold text-gray-900 dark:text-gray-100 mb-2">Поставщики не найдены</h3>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6">
            {suppliers.map(s => (
              <Link
                key={s.id}
                href={s.userId ? `/profile/${s.userId}` : '#'}
                className="card-base p-6 hover-lift group"
              >
                <div className="flex items-start gap-4 mb-4">
                  {s.avatar ? (
                    <div className="w-14 h-14 rounded-2xl overflow-hidden relative shrink-0">
                      <Image src={s.avatar} alt={s.displayName} fill className="object-cover" sizes="56px" unoptimized />
                    </div>
                  ) : (
                    <div className="w-14 h-14 bg-gradient-to-br from-brand-400 to-orange-500 rounded-2xl flex items-center justify-center text-white text-lg font-bold shrink-0">
                      {s.companyName.charAt(0)}
                    </div>
                  )}
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <h3 className="font-bold text-lg group-hover:text-brand-600 transition-colors truncate">{s.companyName}</h3>
                      {s.isVerified && (
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      {s.isPremium && <span className="badge-premium text-[10px]">PREMIUM</span>}
                      {s.isPro && !s.isPremium && <span className="badge-pro text-[10px]">PRO</span>}
                    </div>
                  </div>
                </div>
