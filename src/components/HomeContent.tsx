...
function AnimatedCounter({ value }: { value: number }) {
  ...
  return <div ref={ref} className="text-4xl md:text-5xl font-bold text-brand-500 dark:text-brand-400 tabular-nums">{count}</div>;
}
    ...
    <div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-card p-8 md:p-10">
          <div className="grid grid-cols-2 md:grid-cols-5 gap-8 md:gap-4">
            ...
            <div className="text-center">
              ...
              <div className="text-[#717171] dark:text-gray-400 mt-2 font-medium">Специалистов</div>
            </div>
            <div className="text-center">
              <AnimatedCounter value={stats.users} />
              <div className="text-[#717171] dark:text-gray-400 mt-2 font-medium">Пользователей</div>
            </div>
          </div>
        </div>
      </section>
      ...
      {recentImages.length > 0 && (
        <section className="section-padding section-container">
          <div className="flex items-end justify-between mb-10">
            ...
            <Link href="/gallery" className="hidden sm:inline-flex items-center gap-1.5 text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300 font-semibold transition-colors">
              Смотреть все
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 md:gap-6">
            {recentImages.map((img) => (
              <Link key={img.id} href={`/gallery/${img.id}`} className="card-base overflow-hidden group">
                <div className="relative bg-gradient-to-br from-brand-50 via-orange-50 to-amber-50 dark:from-gray-700 dark:via-gray-700 dark:to-gray-700 h-36 md:h-40 overflow-hidden group-hover:from-brand-100 group-hover:to-orange-100 dark:group-hover:from-gray-600 dark:group-hover:to-gray-600 transition-all duration-300">
                  {img.url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center text-brand-200 dark:text-brand-500">
                ...
                <div className="p-3.5">
                  <h3 className="font-semibold text-sm truncate group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">{img.title}</h3>
                  <div className="flex items-center justify-between mt-2">
                    <span className="text-xs text-gray-400 dark:text-gray-500 font-medium">{img.downloads} загрузок</span>
          ...
          <Link href="/gallery" className="sm:hidden flex items-center justify-center gap-1.5 text-brand-600 dark:text-brand-400 font-semibold mt-6">
      ...
      {feedPosts.length > 0 && (
        <section className="section-padding section-container">
          <div className="flex items-end justify-between mb-10">
            ...
            <Link href="/feed" className="hidden sm:inline-flex items-center gap-1.5 text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300 font-semibold transition-colors">
              Смотреть все
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2"><path d="M9 5l7 7-7 7"/></svg>
            </Link>
          </div>
          <div className="max-w-2xl mx-auto space-y-6">
            {feedPosts.map((post) => {
              ...
              const categoryLabels: Record<string, { label: string; color: string }> = {
                news: { label: 'Новость', color: 'bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-100 dark:border-blue-500/20' },
                project: { label: 'Проект', color: 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border-emerald-100 dark:border-emerald-500/20' },
                article: { label: 'Статья', color: 'bg-purple-50 dark:bg-purple-500/10 text-purple-600 dark:text-purple-400 border-purple-100 dark:border-purple-500/20' },
                product: { label: 'Товар', color: 'bg-amber-50 dark:bg-amber-500/10 text-amber-600 dark:text-amber-400 border-amber-100 dark:border-amber-500/20' },
              };
                ...
                <article key={post.id} className="card-base overflow-hidden">
                  ...
                  <div className="px-4 pt-3 pb-2">
                    <Link href={`/feed/${post.id}`}>
                      <h3 className="font-bold text-gray-900 dark:text-gray-100 mb-1.5 hover:text-brand-600 dark:hover:text-brand-400 transition-colors leading-snug line-clamp-2">{post.title}</h3>
                    </Link>
                    <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 leading-relaxed">{post.content}</p>
                  </div>
                  {postImages.length > 0 && (
                  ...
                  {tags.length > 0 && (
                    <div className="px-4 pt-2.5 flex flex-wrap gap-1.5">
                      {tags.slice(0, 3).map(tag => <span key={tag} className="text-[11px] text-brand-600 dark:text-brand-400 bg-brand-50 dark:bg-brand-500/10 px-2 py-0.5 rounded-full font-medium">#{tag}</span>)}
                    </div>
                  )}
                  <div className="flex items-center gap-1 px-4 py-2.5 mt-1">
                    <Link href={`/feed/${post.id}`} className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-400 dark:text-gray-500 hover:text-red-500 dark:hover:text-red-400 transition-colors rounded-lg hover:bg-red-50 dark:hover:bg-red-500/10">
                    ...
                    <Link href={`/feed/${post.id}`} className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-400 dark:text-gray-500 hover:text-brand-500 dark:hover:text-brand-400 transition-colors rounded-lg hover:bg-brand-50 dark:hover:bg-brand-500/10">
                    ...
                    <span className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-400 dark:text-gray-500">
          ...
          <Link href="/feed" className="sm:hidden flex items-center justify-center gap-1.5 text-brand-600 dark:text-brand-400 font-semibold mt-6">
            Смотреть все новости →
          </Link>
        </section>
      )}

      <section className="section-padding bg-[#f0ede7] dark:bg-gray-800">
      ...
      {popularDocs.length > 0 && (
        <section className="section-padding section-container">
          <div className="flex items-end justify-between mb-10">
            ...
            <Link href="/documents" className="hidden sm:inline-flex items-center gap-1.5 text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300 font-semibold transition-colors">
              Смотреть все
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2"><path d="M9 5l7 7-7 7"/></svg>
            </Link>
          </div>
          <div className="space-y-3">
            {popularDocs.map(doc => {
              const iconColor = doc.fileType === 'pdf' ? 'bg-red-50 dark:bg-red-500/10 text-red-500 dark:text-red-400' : doc.fileType === 'xlsx' ? 'bg-emerald-50 dark:bg-emerald-500/10 text-emerald-500 dark:text-emerald-400' : 'bg-blue-50 dark:bg-blue-500/10 text-blue-500 dark:text-blue-400';
              return (
                <Link key={doc.id} href={`/documents/${doc.id}`} className="card-base flex items-center justify-between p-5 hover-lift">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h3 className="font-semibold text-gray-900 dark:text-gray-100 group-hover:text-brand-600 dark:group-hover:text-brand-400 transition-colors">{doc.title}</h3>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <div key={i} className="card-base p-7">
                ...
                <p className="text-[#6B6B6B] dark:text-gray-400 leading-relaxed mb-5">&ldquo;{t.text}&rdquo;</p>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-[#717171] dark:text-gray-400">{t.role}</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      <section className="relative overflow-hidden py-20" style={{ background: 'linear-gradient(160deg, #111827 0%, #1f2937 50%, #374151 100%)' }}>
        ...
        <div className="relative section-container text-center">
          ...
          <Link href="/register" className="inline-flex items-center gap-2.5 px-10 py-4 bg-white text-gray-900 dark:bg-gray-700 dark:text-gray-100 font-bold rounded-2xl shadow-float hover:shadow-2xl hover:scale-[1.02] active:scale-[0.98] transition-all duration-200">
            Начать бесплатно
