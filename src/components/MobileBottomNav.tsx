          ...
          >
            ...
            {userId && (
              <div className="px-4 mb-3">
                <p className="text-[11px] font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-2 px-1">Личный кабинет</p>
                <div className="space-y-0.5">
                  {[
                    ...
                    { href: '/dashboard/promotion', label: 'Продвижение', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="1.5"><path d="M13 10V3L4 14h7v7l9-11h-7z"/></svg> },
                    ...
                    >
                      <span className={pathname === link.href ? 'text-brand-500' : 'text-gray-400 dark:text-gray-500'}>{link.icon}</span>
                      <span className="text-sm font-medium">{link.label}</span>
                    </Link>
                  ))}
                  {userRole === 'ADMIN' && (
      ...
      <nav className="md:hidden fixed bottom-0 left-0 w-screen bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-t border-gray-200 dark:border-gray-800 z-[60] safe-area-bottom" style={{ position: 'fixed' }}>
        <div className="flex items-stretch px-1">
          ...
          <button
            ...
            className={`flex-1 flex flex-col items-center justify-center gap-0.5 py-2 transition-all duration-200 min-h-[60px] relative ${
