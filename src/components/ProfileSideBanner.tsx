...
export default function ProfileSideBanner({ banner, position }: ProfileSideBannerProps) {
  ...
  if (!isFilled) {
    return (
      <div className="rounded-xl border-2 border-dashed border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-800/50 p-4 flex flex-col items-center justify-center text-center min-h-[180px]">
  ...
  const content = (
    <div className="relative rounded-xl overflow-hidden min-h-[180px] group cursor-pointer">
      <Image src={banner.imageUrl} alt={banner.title || 'Баннер'} fill className="object-cover group-hover:scale-105 transition-transform duration-300" sizes="250px" unoptimized />
